...
% type 2 model and systematically vary the parameter k, which controls
% the type 2 signal loss, while holding sd2 (which controls the standard 
% deviation of type 2 noise) constant at a non-zero value.
%
% this scripts mirrors the analaysis of the type 2 signal loss model 
% conducted in  the main manuscript, as displayed in Figure 10.
%
...
ks   = linspace(0,.9,11);     % k is signal loss, must be in [0,1]. 0 --> no signal loss, 1 --> complete signal loss.
sd2s = .1*ones(size(ks));     % sd2 is std dev of type 2 noise, must be >= 0.
...
t2titlestr = ['using type 2 signal loss model, varying k, fixing \sigma_2 = ' num2str(sd2s(1))];
...
if save_data
    ...
    filename = ['k_' type '_d=' num2str(param.d) '_c=' num2str(param.c) '_pS2=' num2str(param.pS2)];
    ...
    if settings.compute_metad
        filename_M = ['k_Mratio_d=' num2str(param.d) '_c=' num2str(param.c) '_pS2=' num2str(param.pS2) '_nt=1e' num2str(log10(settings.ntrials))];
...
%% plot optimal type 2 criteria as a function of k
% plots optimal type 2 criteria as a function of type 2 signal loss k for each
% response type. the optimal type 2 criterion when ignoring type 2 signal loss
% and using classical SDT corresponds to the point on the plot where k = 0
...
%% plot optimal type 2 performance as a function of k
% plots type 2 performance as a function of type 2 signal loss k for each
...
% plot "S1" response results
subplot(1,2,1); hold on;
plot(ks, t2perf_rS1_sim, 'o-');
plot(ks, t2perf_rS1_calc, 'o-');
...
% plot "S2" response results
subplot(1,2,2); hold on;
plot(ks, t2perf_rS2_sim, 'o-'); 
plot(ks, t2perf_rS2_calc, 'o-');
...
% 
% M_ratio is controlled by k such that as k increases, M_ratio decreases.
...
if settings.compute_metad
    ...
    ylabel('optimal c_2 for "S1" responses')
    xlabel('M_{ratio} as controlled by k')
    ylim(yminmax)
...
%% plot M_ratio as a function of k
% plots M_ratio as a function of k for each response type
